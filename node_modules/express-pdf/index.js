...
function sendHTMLPDF(res, filename, content, options, save){
    
        if(save){
            pdf.create(content, options).toFile('./'+filename,function(err, stream){
                if(err){
                    reject(err);
                }else{
                    console.log(stream);
    ...
    return new Promise(function (resolve, reject) {
        ...
        if(opt.html !== undefined){
            fs.readFile(opt.html, 'utf-8', function(err, data){
                ...
                sendHTMLPDF(_this, opt.filename, data, opt.options, opt.save)
                .then(resolve, reject);
            });
        }else if(opt.htmlContent !== undefined){
            sendHTMLPDF(_this, opt.filename, opt.htmlContent, opt.options, opt.save)
            .then(resolve, reject);
        }else{
            internalError(_this);
